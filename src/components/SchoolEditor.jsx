...
const PRODUCT_OPTIONS = ['Camisetas', 'Bermudas', 'Shorts Saia', 'Moletom', 'Calça Menino', 'Calça Menina'];
    ...
    return (
        <div className="card" style={{ marginBottom: '0.875rem' }}>
            <div style={{
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--primary)', fontWeight: 600, background: 'rgba(0,113,227,0.08)', padding: '3px 10px', borderRadius: '980px', border: '1px solid rgba(0,113,227,0.15)' }}>
                    {totalQty} pçs
                </span>
            </div>
...
export default function SchoolEditor({ school, franchise, onSave, onBack, onDelete }) {
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', paddingBottom: '3rem' }}>
            {/* Header */}
            <div className="card" style={{ marginBottom: '1.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    ...
                    {franchise.id && (
                        <button className="btn btn-secondary" style={{ color: 'var(--danger)', borderColor: 'rgba(255,59,48,0.25)', background: 'var(--danger-bg)' }} onClick={() => onDelete(franchise.id)}>
                            <Trash2 size={18} />
                        </button>
                    )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr', gap: '2rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    {/* Products Section */}
                    <div>
                        <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--text-main)', fontWeight: 700, letterSpacing: '-0.03em' }}>
                            Produção <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 500 }}>({totalItems} itens)</span>
                        </h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {currentProducts.length === 0 && (
                                <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '3rem', border: '2px dashed var(--border-color)', borderRadius: '14px', background: 'white' }}>
                                    <Shirt size={40} color="var(--text-tertiary)" style={{ marginBottom: '0.75rem' }} />
                                    <p style={{ margin: 0, fontSize: '0.9rem' }}>Nenhum item adicionado ainda.</p>
                                </div>
                            )}
                ...
                <div>
                    ...
                    <div className="card" style={{ marginBottom: '1rem' }}>
                        <h3 style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.875rem', paddingBottom: '0.875rem', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '7px' }}>
                            <Check size={14} /> Status do Pedido
                        </h3>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            <button
                                type="button"
                                className="btn btn-secondary"
                                onClick={() => { setFormData(prev => ({ ...prev, financials: { ...prev.financials, picked_up: !prev.financials.picked_up } })); setIsDirty(true); }}
                                style={{
                                    width: '100%', justifyContent: 'center', borderRadius: '10px',
                                    background: formData.financials.picked_up ? 'var(--success-bg)' : '',
                                    borderColor: formData.financials.picked_up ? 'var(--success)' : '',
                                    color: formData.financials.picked_up ? 'var(--success-text)' : 'var(--text-muted)',
                                    fontWeight: formData.financials.picked_up ? 600 : 400
                                }}
                            >
                                <Check size={15} /> Pegamos o Pedido
                            </button>

                            <button
                                type="button"
                                className="btn btn-secondary"
                                onClick={() => { setFormData(prev => ({ ...prev, financials: { ...prev.financials, delivered: !prev.financials.delivered } })); setIsDirty(true); }}
                                style={{
                                    width: '100%', justifyContent: 'center', borderRadius: '10px',
                                    background: formData.financials.delivered ? 'rgba(0,113,227,0.08)' : '',
                                    borderColor: formData.financials.delivered ? 'var(--primary)' : '',
                                    color: formData.financials.delivered ? 'var(--primary)' : 'var(--text-muted)',
                                    fontWeight: formData.financials.delivered ? 600 : 400
                                }}
                            >
                                <Check size={15} /> Pedido Entregue
                            </button>

                            <button
                                type="button"
                                className="btn btn-secondary"
                                onClick={() => { setFormData(prev => ({ ...prev, financials: { ...prev.financials, fully_paid: !prev.financials.fully_paid } })); setIsDirty(true); }}
                                style={{
                                    width: '100%', justifyContent: 'center', borderRadius: '10px',
                                    background: formData.financials.fully_paid ? 'var(--success-bg)' : '',
                                    borderColor: formData.financials.fully_paid ? 'var(--success)' : '',
                                    color: formData.financials.fully_paid ? 'var(--success-text)' : 'var(--text-muted)',
                                    fontWeight: formData.financials.fully_paid ? 600 : 400
                                }}
                            >
                                <DollarSign size={15} /> 100% Pago
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="card" style={{ position: 'sticky', top: '2rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {[
                                { label: 'VALOR COBRADO (TOTAL)', name: 'total_value', value: formData.financials.total_value },
                                { label: 'VALOR PAGO (SINAL)', name: 'value_paid', value: formData.financials.value_paid },
                                { label: 'CUSTO DE PRODUÇÃO', name: 'production_cost', value: formData.financials.production_cost }
                            ].map(field => (
                                <div key={field.name}>
                                    <label style={{ display: 'block', fontSize: '0.68rem', fontWeight: 600, marginBottom: '0.4rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{field.label}</label>
                                    <div style={{ position: 'relative' }}>
                                        <span style={{ position: 'absolute', left: '11px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-tertiary)', fontSize: '0.85rem', fontWeight: 500, pointerEvents: 'none' }}>R$</span>
                                        <input className="input" style={{ paddingLeft: '2.25rem' }} name={field.name} value={field.value || ''} onChange={handleFinancialChange} placeholder="0.00" />
                                    </div>
                                </div>
                            ))}

                            <div style={{ padding: '1.1rem 1.25rem', background: profit >= 0 ? 'var(--success-bg)' : 'var(--danger-bg)', borderRadius: '12px', border: `1px solid ${profit >= 0 ? 'rgba(52,199,89,0.25)' : 'rgba(255,59,48,0.2)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                                <div style={{ fontSize: '0.7rem', color: profit >= 0 ? 'var(--success-text)' : 'var(--danger-text)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Lucro Estimado</div>
                                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: profit >= 0 ? 'var(--success-text)' : 'var(--danger)', letterSpacing: '-0.04em' }}>
                                    R$ {profit.toFixed(2)}
                                </div>
                            </div>
                        </div>
