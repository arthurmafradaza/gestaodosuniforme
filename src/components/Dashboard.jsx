...
export default function Dashboard({ schools, currentPath, onNavigate, onAddFolder, onDeleteFolder, onUpdateSchool, onRenameSchool, isEmbedded = false }) {
    ...
    const DonutChart = ({ data }) => {
        ...
        const colors = ['#0071e3', '#5856d6', '#30d158', '#ff9f0a', '#ff3b30', '#bf5af2'];
        const segments = [
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', justifyContent: 'center' }}>
                <div className="donut-container">
                    ...
                    <div className="progress-text">
                        <div style={{ fontSize: '0.62rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '2px' }}>Total</div>
                ...
                <div className="analytics-view">
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '0.4rem' }}>
                                    ...
                                    <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Atualizado agora</span>
                                </div>
                                <h1 className="page-title" style={{ fontSize: '2.5rem', letterSpacing: '-0.05em' }}>Painel Executivo</h1>
                                <p style={{ color: 'var(--text-muted)', marginTop: '0.35rem', marginBottom: 0, fontSize: '0.9rem' }}>Saúde financeira e fluxo de produção.</p>
                            </div>
                            <div style={{ display: 'flex', gap: '1.5rem', paddingBottom: '0.5rem' }}>
                                <div style={{ textAlign: 'center', padding: '1rem 1.5rem', background: 'white', borderRadius: '14px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-xs)' }}>
                                    <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Escolas</div>
                                    <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--primary)', letterSpacing: '-0.04em' }}>{schools.length}</div>
                                </div>
                                <div style={{ textAlign: 'center', padding: '1rem 1.5rem', background: 'white', borderRadius: '14px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-xs)' }}>
                                    <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Unidades</div>
                                    <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--accent)', letterSpacing: '-0.04em' }}>{allFranchises.length}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: '2rem' }}>
                        <div className="card stat-card" style={{ borderTop: '3px solid var(--primary)' }}>
                            <div style={{ position: 'absolute', top: '1.1rem', right: '1.1rem', padding: '7px', background: 'rgba(0,113,227,0.08)', borderRadius: '9px' }}>
                            ...
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.3rem' }}>Total contratado</div>
                        </div>
                        <div className="card stat-card" style={{ borderTop: '3px solid var(--danger)' }}>
                            <div style={{ position: 'absolute', top: '1.1rem', right: '1.1rem', padding: '7px', background: 'var(--danger-bg)', borderRadius: '9px' }}>
                            ...
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.3rem' }}>
                                {totalRevenue > 0 ? `${((totalCost / totalRevenue) * 100).toFixed(1)}% do faturamento` : 'Aguardando dados'}
                            </div>
                        </div>
                        <div className="card stat-card" style={{ borderTop: '3px solid var(--success)' }}>
                            <div style={{ position: 'absolute', top: '1.1rem', right: '1.1rem', padding: '7px', background: 'var(--success-bg)', borderRadius: '9px' }}>
                                <Wallet size={16} color="var(--success-text)" />
                            </div>
                            <div className="stat-label">LUCRO LÍQUIDO</div>
                            <div className="stat-value" style={{ color: totalProfit >= 0 ? 'var(--success-text)' : 'var(--danger)' }}>
                                R$ {totalProfit.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                            </div>
                            <div style={{ fontSize: '0.72rem', color: 'var(--success-text)', marginTop: '0.3rem', fontWeight: 600 }}>
                                {totalRevenue > 0 ? `Margem: ${((totalProfit / totalRevenue) * 100).toFixed(1)}%` : 'Margem: —'}
                            </div>
                        </div>
                        <div className="card stat-card" style={{ borderTop: '3px solid var(--accent)' }}>
                            <div style={{ position: 'absolute', top: '1.1rem', right: '1.1rem', padding: '7px', background: 'rgba(88,86,214,0.08)', borderRadius: '9px' }}>
                                <Shirt size={16} color="var(--accent)" />
                            </div>
                            <div className="stat-label">EM PRODUÇÃO</div>
                            <div className="stat-value">{totalItemsCount.toLocaleString()} <span style={{ fontSize: '0.9rem', fontWeight: 500, color: 'var(--text-muted)' }}>pçs</span></div>
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.3rem' }}>
                                {allFranchises.length > 0 ? `~${Math.round(totalItemsCount / Math.max(allFranchises.length, 1))} por unidade` : 'Sem unidades'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <span style={{ fontSize: '0.68rem', fontWeight: 600, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Análise Detalhada</span>
                        <div style={{ height: '1px', flex: 1, background: 'var(--border-color)' }} />
                    </div>
                    <div className="analytics-container">
                        <div className="card chart-card">
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: '1px solid var(--border-subtle)' }}>
                                <div style={{ padding: '6px', background: 'rgba(0,113,227,0.08)', borderRadius: '8px' }}>
                                    <TrendingUp size={15} color="var(--primary)" />
                                </div>
                                <div>
                                    <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-main)' }}>DRE — Composição de Custos</div>
                                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Detalhamento por categoria</div>
                                </div>
                            </div>
                        ...
                        <div className="card chart-card">
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: '1px solid var(--border-subtle)' }}>
                                <div style={{ padding: '6px', background: 'rgba(88,86,214,0.08)', borderRadius: '8px' }}>
                                    <Shirt size={15} color="var(--accent)" />
                                </div>
                                <div>
                                    <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-main)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Produção por Escola</div>
                                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Top 5 em volume de peças</div>
                                </div>
                            </div>
                ...
                <div className="management-view">
                            ...
                            <div className="breadcrumb-nav">
                                ...
                                {parentSchool && (
                                    <>
                                        ...
                                        <span className="breadcrumb-item" style={{ color: 'var(--primary)', background: 'rgba(0, 113, 227, 0.08)', fontWeight: 600 }}>{parentSchool.name}</span>
                                    </>
                                )}
                            </div>
                    ...
                    {currentLevel === 'school' && (
                        <div className="stats-grid" style={{ marginBottom: '2rem' }}>
                            <div className="card stat-card" onClick={() => openEditSchoolModal('total_value')} style={{ cursor: 'pointer', borderTop: '3px solid var(--primary)' }}>
                                <div className="stat-label">FATURAMENTO <Pencil size={11} style={{ marginLeft: 'auto', color: 'var(--text-tertiary)' }} /></div>
                                <div className="stat-value">R$ {totalRevenue.toLocaleString()}</div>
                            </div>
                            <div className="card stat-card" onClick={() => openEditSchoolModal('entry_value')} style={{ cursor: 'pointer', borderTop: '3px solid #06b6d4' }}>
                                <div className="stat-label">VALOR ENTRADA <Pencil size={11} style={{ marginLeft: 'auto', color: 'var(--text-tertiary)' }} /></div>
                                <div className="stat-value">R$ {totalEntryValue.toLocaleString()}</div>
                            </div>
                            <div className="card stat-card" onClick={() => openEditSchoolModal('production_cost')} style={{ cursor: 'pointer', borderTop: '3px solid var(--danger)' }}>
                                <div className="stat-label">CUSTO <Pencil size={11} style={{ marginLeft: 'auto', color: 'var(--text-tertiary)' }} /></div>
                    ...
                    {itemsToDisplay.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '4rem', background: 'white', borderRadius: '16px', border: '2px dashed var(--border-color)' }}>
                            <div style={{ background: 'var(--bg-app)', width: '64px', height: '64px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
                                <Folder size={28} color="var(--text-tertiary)" />
                            </div>
                            <p style={{ margin: 0, fontSize: '0.9rem' }}>Nenhuma {currentLevel === 'root' ? 'escola' : 'unidade'} encontrada.</p>
                        </div>
                    ) : (
                        <div className="grid-folders">
                            {itemsToDisplay.map((item) => (
                                ...
                                >
                                    ...
                                    {currentLevel === 'root' ?
                                        <div style={{ padding: '10px', background: 'rgba(0, 113, 227, 0.08)', borderRadius: '10px' }}>
                                            <Building2 size={26} color="var(--primary)" />
                                        </div>
                                        :
                                        <div style={{ padding: '10px', background: 'rgba(88, 86, 214, 0.08)', borderRadius: '10px' }}>
                                            <Folder size={26} color="var(--accent)" />
                                        </div>
                                    }
            ...
            {isEditSchoolModalOpen && (
                <div className="modal-overlay" onClick={() => setIsEditSchoolModalOpen(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ background: 'var(--warning-bg)', borderLeft: '3px solid var(--warning)', padding: '0.75rem 1rem', marginBottom: '1.5rem', borderRadius: '8px' }}>
                            <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--warning-text)' }}>
